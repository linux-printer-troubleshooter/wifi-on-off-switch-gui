...
your_title=""
#add a custom header if you so desire
header=""
...
if [[ $status -eq 20 ]]
then
arg=$(zenity --forms --title="$your_title" --text="$header" --add-list="$name" --list-values="Off|On" --column-values="Select");
else
arg=$(zenity --forms  --title="$your_title" --text="$header" --add-list="$name" --list-values="On|Off" --column-values="Select");
fi;
